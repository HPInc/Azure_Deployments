...
locals {
  resource_group_name = var.resource_group_name != "" ? var.resource_group_name : "lb_oneip_deployment_${random_id.deployment-name.hex}"
}
...
module "load-balancer" {
  ...
  instance_count            = length(var.cac_configuration)
  prefix                    = var.prefix
  resource_group_name       = azurerm_resource_group.main.name
  location                  = azurerm_resource_group.main.location
}
...
module "cac-network" {
  source = "../../modules/cac/cac-network-lb-nat"
...
module "cac-vm" {
  source = "../../modules/cac/cac-vm-lb-nat"
...
module "cac-configuration" {
  ...
  cac_ips                     = module.load-balancer.cac-public.*.ip_address
