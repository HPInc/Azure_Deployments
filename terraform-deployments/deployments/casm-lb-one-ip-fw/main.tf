...
module "load-balancer" {
  source = "../../modules/network/casm-load-balancer-one-ip"
  
  load_balancer_depends_on = [module.cac.public-ip]
  instance_count            = var.cac_count
  prefix                    = var.prefix
  resource_group_name       = azurerm_resource_group.main.name
  location                  = azurerm_resource_group.main.location
  cac_network_interface_ids = module.cac.network-interface-nic-ids[0]
}
...
module "cac" {
  ...
  cac_depends_on = azurerm_storage_account.storage
