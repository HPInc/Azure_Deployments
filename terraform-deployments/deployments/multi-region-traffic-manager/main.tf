...
locals {
  resource_group_name = var.resource_group_name != "" ? var.resource_group_name : "traffic_manager_deployment_${random_id.deployment-name.hex}"
}
...
module "traffic-manager" {
  ...
  resource_group_name    = azurerm_resource_group.main.name
  managed_endpoints      = module.cac-network.cac-public-ips
  managed_endpoint_names = module.cac-vm.cac-vm-names
