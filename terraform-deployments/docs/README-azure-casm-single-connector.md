#  AWM Single-Connector Deployment (Standalone with AAD DS)
...
## Table of Contents
1. [AWM-Single-Connector Architecture](#1-awm-single-connector-architecture)
...
7. [Deploying the Single-Connector via Terraform](#7-deploying-the-awm-single-connector-via-terraform)
...
### 1. AWM-Single-Connector Architecture
...
This deployments runs the Anyware Manager in a virtual machine which gives users full control of Anyware deployment. The Anyware deployment will not have to reach out to the internet for Anyware management features, but the user is responsible for costs, security, updates, high availability and maintenance of the virtual machine running Anyware Manager.
...
- [Git](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git/)d
...
1. Upon completion of the deploy script, the Azure Cloud Shell will output the required Application ID and Client Secret which must be saved in the terraform.tfvars file
 ```
  {
  "appId": "xxxxxxxxxx",
  "displayName": "casm_single_connector",
  "password": "xxxxxxxxxx",
  "tenant": "xxxxxxxxxx"
  }
 ```
...
### 7. Deploying the AWM-Single-Connector via Terraform
...
Terraform is a declarative language to describe the desired state of resources. A user can modify terraform.tfvars and run ```terraform apply``` again. Terraform will try to only apply the changes needed to achieve the new state.
