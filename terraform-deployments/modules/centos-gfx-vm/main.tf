...
resource "azurerm_linux_virtual_machine" "centos-gfx-vm-im" {
...
resource "azurerm_linux_virtual_machine" "centos-gfx-vm-sp" {
...
resource "azurerm_virtual_machine_extension" "centos-gfx-provisioning" {
  ...
  depends_on = [azurerm_linux_virtual_machine.centos-gfx-vm-im, azurerm_linux_virtual_machine.centos-gfx-vm-sp]

  for_each             = var.workstations
  name                 = each.value.prefix == "" ? "gcent-${each.value.index}" : "${each.value.prefix}-gcent-${each.value.index}"
  virtual_machine_id   = var.managed_identity_id != "" ? azurerm_linux_virtual_machine.centos-gfx-vm-im[each.key].id : azurerm_linux_virtual_machine.centos-gfx-vm-sp[each.key].id
