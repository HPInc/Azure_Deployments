...
resource "azurerm_network_interface" "centos-std-nic" {
  ...
  name                = "centos-std-${each.value.index}-nic"
  location            = each.value.location
  resource_group_name = var.resource_group_name
  ip_configuration {
...
resource "azurerm_linux_virtual_machine" "centos-std-vm" {
  ...
  name                            = "${each.value.prefix}-scent-${each.value.index}"
  resource_group_name             = var.resource_group_name
  location                        = each.value.location
...
resource "azurerm_virtual_machine_extension" "centos-std-provisioning" {
  ...
  {
  "script": "${base64encode(templatefile("${path.module}/${local.centos_std_provisioning_script}.tmpl", {
