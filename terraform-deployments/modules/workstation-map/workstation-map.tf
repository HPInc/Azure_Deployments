...
locals {
  ...
  centos_std_workstations_map = {
    for count, item in local.centos_std_workstations_map_no_count : "${item.workstation_os}_host_${count}" => {
      index          = tonumber(count),
      location       = item.location,
      prefix         = item.prefix,
      workstation_os = item.workstation_os,
  ...
  centos_gfx_workstations_map = {
    for count, item in local.centos_gfx_workstations_map_no_count : "${item.workstation_os}_host_${count}" => {
      index          = tonumber(count),
      location       = item.location,
      prefix         = item.prefix,
      workstation_os = item.workstation_os,
